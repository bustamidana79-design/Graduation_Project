...
- لا تستخدم رموزا غريبة أو زخارف أو أحرفا غير عربية.
    ...
    await saveMessage({
      supabase,
      table: "ai_chat_messages",
      payload: { session_id: currentSessionId, role: "assistant", message: reply },
    });
    ...
    return NextResponse.json({ reply, analysis: analysisResult, sessionId: currentSessionId, userType });
  } catch (error) {
