...
export default function SmallBusinessFavoritesPage() {
  ...
  return (
    <div className="space-y-6 p-6" dir="rtl">
      ...
      {loading ? (
        <div className="rounded-lg border border-[#e6edf5] bg-white p-6 text-sm text-[#273347]/60">جاري تحميل المفضلة...</div>
      ) : favorites.length === 0 ? (
        ...
        <div className="grid gap-5 md:grid-cols-2 xl:grid-cols-3">
          {favorites.map((row) => {
            ...
            return (
              <article key={row.id} className="overflow-hidden rounded-lg border border-[#e6edf5] bg-white">
                <Link href={`/dashboard/small-business/products/${product.id}`} className="block bg-[#f8fafc]">
                  <img src={getPublicImage(image?.image_url)} alt={product.name} className="h-52 w-full object-contain" />
                </Link>
                <div className="space-y-3 p-5">
                  <div>
                    <Link href={`/dashboard/small-business/products/${product.id}`} className="text-lg font-bold text-[#273347] hover:underline">
                      {product.name}
                    </Link>
                    <p className="mt-1 text-sm text-[#546a85]">
                      السعر: {isConverted ? "≈ " : ""}{formatMoney(convertedPrice, userCurrency)}
                    </p>
                  </div>
                  ...
                  <div className="flex gap-3">
                    <button
                      ...
                      className="flex flex-1 items-center justify-center gap-2 rounded-lg border border-red-200 px-4 py-2 text-sm font-semibold text-red-700"
                    >
                      <HeartOff size={16} />
                      إزالة
                    </button>
                    <Link
                      ...
                      className="flex flex-1 items-center justify-center rounded-lg border border-[#bbd0e4] px-4 py-2 text-sm font-semibold text-[#273347]"
                    >
                      عرض المزيد
                    </Link>
                    <button
